...
│   └── security.yml      ← CodeQL + dependency review (separate trigger)
└── dependabot.yml        ← Galaxy collection + Actions version bumps
```
...
jobs:
  ansible-lint:
    ...
    steps:
      # Pin to SHA, not tag — tags are mutable.
...
# Caches the pip install and the Molecule ephemeral instances via Docker layer cache.
...
# Deploys playbook to staging (auto) and production (manual approval).
# Uses OIDC — no long-lived SSH keys or cloud credentials stored in secrets.
...
jobs:
  deploy-staging:
    ...
    steps:
      ...
      # Exchange OIDC token for short-lived SSH certificate via your CA
      # (e.g. Vault SSH secrets engine, Teleport, or Boundary).
      # Replace with your OIDC → SSH cert action of choice.
      - name: Obtain SSH certificate (OIDC)
...
# CodeQL scans Python/YAML; dependency-review blocks vulnerable Galaxy collections.
...
**`dependabot.yml` — Auto-update Actions + Galaxy collections:**

```yaml
# .github/dependabot.yml
# Weekly PRs for GitHub Actions SHA bumps and pip (Ansible tools) updates.
# Galaxy collections must be bumped manually via requirements.yml — Dependabot
# does not yet natively support ansible-galaxy, so use Renovate Bot instead.
...
resource "vault_jwt_auth_backend_role" "ansible_deploy_staging" {
  ...
  user_claim = "actor"
  role_type  = "jwt"
  ttl        = "900"    # 15-minute token — shortest viable window
}
```

**Least-privilege GITHUB_TOKEN permissions (set at workflow level):**

```yaml
# Add to every workflow's top-level permissions block.
# Grant only what the job needs; deny everything else implicitly.
permissions:
  contents: read          # checkout
  id-token: write         # OIDC
  pull-requests: write    # PR comments (deploy summary)
  # Do NOT add: packages, deployments, issues unless explicitly needed
```
...
images:
  base_image:
    name: ghcr.io/ansible/community-ee-minimal:2.20.0  # pin to digest in prod
...
**Build and push EE in CI:**

```yaml
# Add to ci.yml or a dedicated ee-build.yml, triggered on requirements.yml change.
      ...
      - name: Build EE
        run: |
          ansible-builder build \
            -t ghcr.io/${{ github.repository_owner }}/clincher-ee:${{ github.ref_name }} \
            --container-runtime docker \
            -v 3

      - name: Push EE image
        run: |
          docker push ghcr.io/${{ github.repository_owner }}/clincher-ee:${{ github.ref_name }}
          # Also tag as SHA for reproducibility
          docker tag \
            ghcr.io/${{ github.repository_owner }}/clincher-ee:${{ github.ref_name }} \
            ghcr.io/${{ github.repository_owner }}/clincher-ee:${{ github.sha }}
          docker push ghcr.io/${{ github.repository_owner }}/clincher-ee:${{ github.sha }}
```
...
├── inventory/
│   ├── staging/
│   │   └── hosts.yml     ← staging targets (no real IPs in git; use vars)
│   └── production/
...
There is no Flux/ArgoCD equivalent that works natively with Ansible playbooks (those tools target Kubernetes manifests). The GitOps pattern for Ansible is:
...
jobs:
  check-drift:
    ...
    steps:
      ...
      - name: Check for drift (--check --diff)
        id: drift
        run: |
          set +e
          ansible-playbook playbook.yml \
...
Ansible playbooks managing a **single server** (as in this project) use a simplified deployment model — the concepts of blue-green and canary apply at the **service level** (Docker containers) rather than the infrastructure level.
...
**Blue-green at the Docker layer (in playbook):**

```yaml
# roles/openclaw-deploy/tasks/blue-green.yml
# Zero-downtime deploy: bring up new container before stopping old.
# Caddy or nginx upstream is switched atomically via docker network connect.
...
- name: Switch proxy upstream to green
...
```
# /etc/caddy/Caddyfile — canary: 10% to new, 90% to stable
openclaw.yourdomain.com {
    reverse_proxy {
        to openclaw-stable:3000   # weight 9
        to openclaw-canary:3000   # weight 1
    }
}
```
...
**What to instrument for an Ansible deployment pipeline:**

| Signal | Tool | Where |
|---|---|---|
| Deploy duration | GitHub Actions built-in metrics | Actions → Insights |
| Deploy success/failure rate | GitHub Actions + Slack notify | `on: workflow_run` event |
| Host health post-deploy | Prometheus + Grafana (role: monitoring) | Already in this repo |
| Ansible task failures | ansible-lint SARIF → GitHub Security tab | `security.yml` SARIF upload |
| Drift | Drift detection cron (see §5) | GitHub Issues |

**Post-deploy Slack notification:**

```yaml
# Add as final step in deploy.yml production job.
      ...
      - name: Notify Slack on deploy result
        ...
        with:
          payload: |
            {
              "text": "${{ job.status == 'success' && '✅' || '❌' }} Production deploy *${{ github.ref_name }}* — ${{ job.status }}",
              "blocks": [{
                "type": "section",
                "text": {
                  "type": "mrkdwn",
...
**SARIF upload for ansible-lint findings:**

```yaml
# Add to _lint.yml after ansible-lint step.

      - name: Run ansible-lint (SARIF output)
        run: |
          ansible-lint --profile production -f sarif -o ansible-lint.sarif || true
...
- **GitHub Actions Copilot**: In the Actions editor (`github.com/repo/actions/new`), Copilot suggests complete workflow YAML based on a natural language description. Effective for bootstrapping reusable workflows.
...
```yaml
# .github/ISSUE_TEMPLATE/deploy-failure.yml
name: Deploy Failure
...
- [ ] Post-deploy health check step in `deploy.yml` (curl gateway, check containers)
...
| Molecule not testing idempotency | Role applies changes on every run | Add `idempotency` step: run `converge` twice; assert no changes on second run |
| `become: true` at play level | Entire play runs as root — blast radius too wide | Move `become: true` to individual privileged tasks only |
| Missing `changed_when` on `command` tasks | Always reports changed; breaks idempotency checks | Add `changed_when: false` or parse stdout to detect actual change |
| Hardcoded inventory IPs | Real server IPs in git history | Use `ansible_host: "{{ server_ip }}"` in `hosts.yml`; set `server_ip` in vault or CI env |
| EE image not pinned to digest | `latest` tag silently updates, breaks reproductibility | Pin base image: `name: ghcr.io/ansible/community-ee-minimal@sha256:<digest>` |
| Production deploy without staging gate | Untested change goes straight to prod | `deploy-production` must `needs: deploy-staging` |
...
1. **Migrate to Ansible Execution Environments now** — AAP 2.5+ requires EEs; virtualenvs are deprecated in the automation platform. Getting the `execution-environment.yml` in place today means zero friction when you move to AWX.

2. **OpenTelemetry for Ansible** — the `community.general.opentelemetry` callback plugin (available since ansible-core 2.14) emits traces for every task. Wire it to Grafana Tempo to get flame graphs of playbook execution time. This becomes critical when playbooks grow past 200 tasks.

3. **Renovate Bot over Dependabot for Galaxy** — Dependabot still does not natively update `requirements.yml` Galaxy collection versions (as of March 2026). Renovate Bot with the `ansible-galaxy` datasource handles this. Add `renovate.json` to the repo.
...
5. **GitHub Actions OIDC → Ansible Vault** — the OIDC → Vault pattern shown in `deploy.yml` is the current best practice. Watch for AWS IAM Roles Anywhere and Azure Workload Identity, which offer cloud-native alternatives without running Vault if your infrastructure is cloud-hosted.

6. **Pin `ansible-core` in `requirements.txt`** — `ansible-core` releases have broken backward compatibility in minor versions. Pin to `==2.20.x` and use Dependabot/Renovate to bump deliberately, not silently.
